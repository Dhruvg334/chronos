    ...
    <div className="min-h-screen bg-warm-ivory text-text-primary flex flex-col items-center justify-center p-6">
      <div className="bg-white border border-warm-border p-8 rounded-2xl shadow-sm w-full max-w-md">
        <h2 className="text-3xl font-bold mb-6 text-center">
          Create Chron<span className="text-accent-amber">OS</span> Account
        </h2>
        <form onSubmit={(e) => e.preventDefault()} className="space-y-4">
          <div>
            <label className="block text-sm font-semibold text-text-secondary mb-1">Display Name</label>
            <input
              type="text"
              disabled
        ...
        <p className="mt-4 text-center text-sm text-text-secondary">
          Already have an account? <a href="/login" className="text-accent-amber hover:underline">Log in</a>
        </p>
      </div>
    </div>
  );
