...
class StrategySelector:
    ...
    def recommend(self, context: StrategyContext, preferences: StrategyPreferences | None = None) -> StrategyRecommendation | None:
        ...
        if overloaded and context.major_outcomes >= 1 and enabled(StrategyId.CONSTRAINED_DAY):
            return StrategyRecommendation(strategy=StrategyId.CONSTRAINED_DAY, title="Reduce the weekly load" if context.weekly_planning else "Constrain the day", why="The planned work does not fit the available time.", evidence=(f"{context.remaining_work_minutes} minutes planned", f"{context.free_minutes} minutes free"), action="Protect one major outcome, choose the most useful short tasks, keep essential maintenance, and defer the rest explicitly.", tradeoff="Some work must move; the recommendation favors a credible plan over nominal completeness.", confidence="high", alternatives=(StrategyId.TIME_BLOCKING,))
