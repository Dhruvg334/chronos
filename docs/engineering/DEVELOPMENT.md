...
Core API entry points are `GET /api/v1/today`, `POST /api/v1/today/strategy`, `GET /api/v1/plan`, `POST /api/v1/plan/blocks`, `POST /api/v1/plan/adaptive`, `POST /api/v1/plan/adaptive/{proposal_id}/approve`, `/api/v1/focus-blocks` lifecycle routes, `/api/v1/projects`, `/api/v1/routines`, and the `/api/v1/week` view/proposal/approval routes.
...
Critical writes accept `Idempotency-Key`. Repository adapters call `approve_planning_intake_transaction`, `complete_focus_transaction`, `approve_recovery_transaction`, `approve_adaptive_plan_transaction`, and `approve_weekly_plan_transaction`; do not replace them with sequential REST writes.
