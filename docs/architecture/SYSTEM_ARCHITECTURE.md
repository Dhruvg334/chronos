...
Migration 026 adds the repository-backed context layer described in `CONTEXT_MEMORY.md`. Explicit and proposed memory, knowledge sources, backend-only chunks, hybrid retrieval, and expiring context packs remain user-owned. Adaptive planning and recovery receive bounded context packs when available and retain their existing structured-data path when retrieval degrades. Retrieved content is always untrusted data and cannot authorize a tool or plan mutation.
...
The planning profile also owns onboarding state and explicit personalization. Today applies those preferences to strategy eligibility/frequency, explanation depth, focus-duration choices, recovery thresholds, quick-task batching, and routine continuity. Recommendation feedback uses a dedicated repository and stores only whitelisted compact context. Contextual recovery and stuck guidance remain deterministic-first and never mutate a plan before approval.
...
Migration 019 adds focus lifecycle state; 020 adds the planning profile; 021 adds core idempotent transaction RPCs and operation receipts; 022 adds atomic adaptive-plan approval; 023 hardens its definer boundary; 024 adds projects, outcomes, routines, and weekly plans; 025 adds onboarding and personalization; 026 adds attributable memory, knowledge, retrieval, and context packs; 027 adds normalized integrations, proposals, audits, and hardened Vault access; and 028 adds operational quotas, lifecycle controls, retention, export inventory, and account deletion. Intake approval, focus completion, recovery approval, adaptive-plan approval, weekly-plan approval, knowledge ingestion, external-proposal approval, quota consumption, and lifecycle cleanup use transactional PostgreSQL boundaries.
